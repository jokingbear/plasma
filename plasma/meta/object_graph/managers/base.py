...
class Base:
    ...
    def link(self, *links:tuple[str, str], inplace=False):
        ...
        for context_head, context_tail in links:
            ...
            assert head_namespace in self.graph, f'manager does not contain {head_namespace}'
            assert tail_namespace in self.graph, f'manager does not contain {tail_namespace}'
            assert head_namespace != tail_namespace, f'{head_namespace} is the same as {tail_namespace}'
            
            head_context = Context(self.graph, head_namespace)
            tail_context = Context(self.graph, tail_namespace)            
            ...
            self.graph.add_edge(hid, tid, Link.DELEGATE_TO)
